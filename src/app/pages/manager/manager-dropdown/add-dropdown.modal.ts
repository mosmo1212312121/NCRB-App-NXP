...
@Component({
  selector: 'app-add-dropdown-modal',
  template: `
    <div class="modal-header">
      <h5 class="modal-title pull-left">Add {{ title }}</h5>
    </div>
...
export class AddDropdownModalComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (this.data) {
      this.form = this.fb.group({
        ...
        value: [{ value: this.data.value, disabled: true }, Validators.required],
        groupName: [{ value: this.data.groupName, disabled: true }, Validators.required]
      });
    }
  }
