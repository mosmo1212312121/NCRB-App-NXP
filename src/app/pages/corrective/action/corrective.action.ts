...
@Component({
  ...
  templateUrl: './corrective.action.html'
})
export class CorrectivetActionComponent extends BaseComponent implements OnInit, OnDestroy {
  form: FormGroup;
  ...
  constructor(
    ...
    // initial component
    super(logService);
    this.setPageName('corrective/action');
  ...
  async getData() {
    ...
    if (id) {
      ...
      try {
        this.loading = true;
        const response: ResponseObj = await this.ajax.getCorrectiveActionPCaById(id).toPromise();
  ...
  async selectFile(event) {
    try {
      if (event.target.files.length > 0) {
        for (let i = 0; i < event.target.files.length; i++) {
          ...
          formData.append(
            ...
            `${form.ncrbid}/D5COR/${event.target.files[i].name}/${form.actionId}`
          );
          if (form.ncrbid) {
            ...
            this.attachFiles.push(
              this.fb.group({
                id: null,
                fileName: `${form.ncrbid}/D5COR/${event.target.files[i].name}`,
      ...
      console.error('D5 (Upload File) Errors: ', ex);
    }
  }
  downloadFile(idx: number) {
    ...
    this.bsModalRef = this.modalService.show(ModalFileComponent, {
      ...
      initialState: {
        ...
        folders: [form.ncrbid.toString(), 'D5COR'],
  ...
  async onComplete() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getCorrectiveComplete(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
          response = await this.ajax.updateCorrectiveAction(this.form.getRawValue()).toPromise();
          if (response.status === 200 && response.data) {
            this.form.disable();
        ...
        // On Crashed
        console.error('Corrective Action (Complete) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    }
  ...
  async onPostpone() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getCorrectivePostpone(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
        ...
        // On Crashed
        console.error('Corrective Action (Postpone) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    }
  ...
  async onPostponeApprove() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getCorrectivePostponeApprove(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
  ...
  async onPostponeReject() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getCorrectivePostponeReject(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
          this.form.get('targetDate').patchValue(this.form.get('targetDateOld').value);
          response = await this.ajax.updateCorrectiveAction(this.form.getRawValue()).toPromise();
          if (response.status === 200 && response.data) {
  ...
  get isNCRBOwner() {
    return this.user && this.user?.username === this.form.getRawValue().owner;
  }
