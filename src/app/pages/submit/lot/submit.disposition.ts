...
export class SubmitLotDispositionComponent extends BaseComponent implements OnInit, OnDestroy {
  form: FormGroup;
  ...
  constructor(
    ...
    // initial component
    super(logService);
    this.setPageName('submit/lot/disposition');
    ...
    this.form = this.fb.group({
      ...
      resultBy1: [{ value: null, disabled: true }],
      ...
      resultBy2: [{ value: null, disabled: true }],
      ...
      resultBy3: [{ value: null, disabled: true }],
      disposition: [null]
    });
    this.userSub = this.store.pipe(select('user')).subscribe(async (user: User) => {
      ...
      try {
        ...
        // On Crashed
        console.error('Submit Disposition (Initial) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    });
  ...
  async getData() {
    ...
    if (id) {
      ...
      try {
        this.loading = true;
        const response: ResponseObj = await this.ajax.getLot(parseInt(id, 10)).toPromise();
        if (response.status === 200 && response.data) {
          ...
          if (
            ...
            this.form.get('disposition').value === 'R' ||
            this.form.get('disposition').value === 'E' // Approve or Reject or Engineer (Waiting)
          ) {
            this.form.disable();
          }
        }
  ...
  async onSubmit() {
    ...
    const result = await alertConfirm(`This process will submit result of lot: ${this.form.getRawValue().lotId}.`);
    if (result.value) {
      const id = this.route.snapshot.paramMap.get('id');
      ...
      try {
        ...
        const response: ResponseObj = await this.ajax[
          this.form.getRawValue().problemType === 0 ? 'getLotDispositionForEngineer' : 'getLotDispositionApprove'
        ](
          parseInt(id, 10),
          auth,
        ...
        if (response.status === 200 && response.data) {
          this.form.get('disposition').patchValue(this.form.getRawValue().problemType === 0 ? 'E' : 'Y');
        ...
        // On Crashed
        console.error('Submit Disposition (Submit) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    }
  ...
  async onApprove() {
    ...
    const result = await alertConfirm(`This process will \"Approve\" lot: ${this.form.getRawValue().lotId}.`);
    if (result.value) {
      const id = this.route.snapshot.paramMap.get('id');
      const auth: string = localStorage.getItem('basic_auth');
      try {
  ...
  async onReject() {
    ...
    //   return;
    // }
    const result = await alertConfirm(`This process will \"Reject\" lot: ${this.form.getRawValue().lotId}.`);
    if (result.value) {
      const id = this.route.snapshot.paramMap.get('id');
      const auth: string = localStorage.getItem('basic_auth');
      this.loading = true;
