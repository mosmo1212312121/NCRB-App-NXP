...
export class RequestComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // initial component
    super(logService);
    this.setPageName('requests/list');
  ...
  async ngOnInit() {
    try {
      ...
      // On Crashed
      console.error('Requests (Initial) Errors: ', ex);
    } finally {
      setTimeout(() => {
        this.loading = false;
      }, 50);
  ...
  public async onSearching(searching: any) {
    try {
      this.loading = true;
  ...
  public get title(): string {
    const idx: number = this.parameter.findIndex(o => o.label === 'TITLE_ALL_NC_LIST');
    if (idx > -1) {
