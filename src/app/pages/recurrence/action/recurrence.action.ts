...
@Component({
  selector: 'app-recurrence-action',
  templateUrl: './recurrence.action.html'
})
export class RecurrenceComponent extends BaseComponent implements OnInit, OnDestroy {
  form: FormGroup;
  ...
  constructor(
    ...
    // initial component
    super(logService);
    this.setPageName('recurrence/action');
    ...
    this.userSub = this.store.pipe(select('user')).subscribe(async (user: User) => {
      ...
      try {
        ...
        // On Crashed
        console.error('PreventRecurrence Action (Prepare data) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    });
  ...
  async getData() {
    ...
    if (id) {
      ...
      try {
        this.loading = true;
        const response: ResponseObj = await this.ajax.getPreventRecurrencePrById(id).toPromise();
  ...
  async selectFile(event) {
    try {
      if (event.target.files.length > 0) {
        for (let i = 0; i < event.target.files.length; i++) {
          ...
          formData.append(
            ...
            `${form.ncrbid}/D7PRE/${event.target.files[i].name}/${form.actionId}`
          );
          if (form.ncrbid) {
            ...
            this.attachFiles.push(
              this.fb.group({
                id: null,
                fileName: `${form.ncrbid}/D7PRE/${event.target.files[i].name}`,
      ...
      console.error('D7 (Upload File) Errors: ', ex);
    }
  }
  downloadFile(idx: number) {
    ...
    this.bsModalRef = this.modalService.show(ModalFileComponent, {
      ...
      initialState: {
        ...
        folders: [form.ncrbid.toString(), 'D7PRE'],
  ...
  async onComplete() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getPreventRecurrenceComplete(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
          response = await this.ajax.updatePreventRecurrenceAction(this.form.getRawValue()).toPromise();
          if (response.status === 200 && response.data) {
            this.form.disable();
        ...
        // On Crashed
        console.error('PreventRecurrence Action (Complete) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    }
  ...
  async onPostpone() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax.getPreventRecurrencePostpone(parseInt(id, 10), auth).toPromise();
        if (response.status === 200 && response.data) {
        ...
        // On Crashed
        console.error('PreventRecurrence Action (Postpone) Errors: ', ex);
      } finally {
        this.loading = false;
      }
    }
    ...
    if (result.value) {
      ...
      try {
        ...
        let response: ResponseObj = await this.ajax
          .getPreventRecurrencePostponeApprove(parseInt(id, 10), auth)
          .toPromise();
        if (response.status === 200 && response.data) {
  ...
  async onPostponeReject() {
    ...
    if (result.value) {
      ...
      try {
        this.loading = true;
        let response: ResponseObj = await this.ajax
          .getPreventRecurrencePostponeReject(parseInt(id, 10), auth)
          .toPromise();
        if (response.status === 200 && response.data) {
          this.form.get('targetDate').patchValue(this.form.get('targetDateOld').value);
          response = await this.ajax.updatePreventRecurrenceAction(this.form.getRawValue()).toPromise();
